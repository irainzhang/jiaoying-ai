...
| 编号 | 项目 | 状态 | 完成内容与边界 |
|---|---|---|---|
| O01 | 数据来源目录 | 已实现 / 待外部核实 | 道路、合成业务、天气与预案列明出处。地图来源和合成数据的赛事适用性仍需组委会确认。 |
| O02 | 模型与 Agent | 按要求暂缓 | 不接入 DeepSeek 或 Agent；保留契约，使用规则整理和实际调度算法。 |
| O03 | 真实道路与调度统一 | 已实现 | 瑞安局部道路绑定演练点位，遵守单向通行，计算路线并支持点选上报；速度和开放状态为演练假设。 |
| O04 | 公开预案参考卡 | 已实现 | 浙江与瑞安公开预案追溯到文件和条款，仅作流程参考，非路线审批或官方授权。 |
| O05 | 可复现效果评估 | 已实现 / 模型评估暂缓 | 固定情景、同输入基线与优化，记录覆盖、等待、约束校验与耗时；不主张 AI 增益。 |
| O06 | 情景库与演示引导 | 已实现 | 初始、真实道路、封路、新增12人、运力不足、接收点失效；人工演示和 JSON 复盘恢复。 |
| O07 | 指挥快捷建任务与跟进 | 已实现 | Excel / CSV 名单、文字与语音整理为批量需求；整批人工核对后建任务并生成草案，路线再次确认后发布。建任务与核实、安排转移、跟踪完成仍各有入口。 |
| O08 | 现场执行与快捷补报 | 已实现 | 执行端接收已发布任务，按下一步快捷反馈；新增人员可连续说出一批并一次确认补报，沿用当前任务接人点或已选位置。未知特需保留待补，仍支持总量、更正和草稿。 |
| O09 | 资源失效应对 | 已实现 | 执行中车辆或接收点失效可登记重算；已上车保持原车，缺口保留，恢复需人工核实。 |
| O10 | 方案变更影响 | 已实现 | 逐车比较原方案和草案；同级天气不重排，重复请求不重记，关键障碍立即复核。 |
| O11 | 基线与未安排解释 | 已实现 | 逐车、逐组、逐接收点比较，区分运力、适配、容量和通行问题，展示协助人群等待。 |
| O12 | 复盘报告 | 已实现 | 保留指挥批量建任务来源、村级补报、更正、未决事项、人工确认、版本、诊断和来源，可打印与导出 JSON。 |
| O13 | 保存与恢复 | 已实现 | 公开版浏览器保存，本地新版磁盘保存；导入完整校验并撤销旧发布，存储失败不误报成功。 |
| O14 | 天气来源与接口 | 准备完成 / 实况暂缓 | 彩云接口和单位校验已准备，当前只使用明确标识的一小时累计雨量演练。 |
| O15 | 统一版本说明 | 已实现 | 入口、报告与说明共用当前能力目录，旧核对表明确为历史基准。 |
| O16 | 参赛材料 | 框架已准备 | 提供8—10分钟讲解脚本和提交核对清单；最终 PPT、实录视频、本人签名承诺书待定版制作。 |
| O17 | 跨设备协作 | 待部署条件 | GitHub 支持同浏览器双端联动；手机与电脑共享房间仍需额外后端和访问配置。 |
| O18 | 地区配置与规模验证 | 部分实现 | 单次批量最多 100 条，整场最多 500 名有效人员、200 个有效接送组；超限整批拒绝。情景、路网配置与规模实测可复用，多趟、换车、真实 GPS 和跨地区验证待扩展。 |
...
- 人员、姓名、村庄、车辆、接收点用途与道路状态均使用模拟信息。
- 单车单趟，有限途中重规划，不能保证所有规模的全局最优。
- 未连接原叫应系统，未取得其接口或官方授权。
- 公开访问不等于跨设备共享，浏览器清理可能删除本机记录。
