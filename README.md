...
| O07 | 指挥待办与跟进 | 已实现 | 按核实情况、安排转移、跟踪完成分区；优先显示待核实、受阻、到达待核验，其他跟进按需展开。 |
| O08 | 村级上报易用性 | 已实现 | 现场分报情况、我的任务、上报记录；两步上报、当前任务下一步、浅色界面与固定对话入口；保留总数、更正和草稿。 |
...
| O12 | 复盘报告 | 已实现 | 补入村级批次、更正、未决事项、人工确认、版本、计算诊断和来源，可打印与导出 JSON。 |
...
| O18 | 地区配置与规模验证 | 部分实现 | 情景与真实道路配置可复用，提供规模实测；多趟、换车、真实 GPS 和跨地区验证待扩展。 |
