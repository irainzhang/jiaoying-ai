(function(root){
  ...
  function create({store,render,onGenerate,onBridge,onReview,notify}){
    ...
    const voice=root.JiaoyingVoice.create({Recognition:root.SpeechRecognition||root.webkitSpeechRecognition,secure:root.isSecureContext===true,getText:()=>input,onText:text=>{input=text;},onChange:()=>update()});
    function update(){if(isPage()){const scroll=document.getElementById('chat-messages')?.scrollTop;render();const log=document.getElementById('chat-messages');if(log&&scroll!==undefined)log.scrollTop=scroll;}}
    function reset(){voice.cancel('点击开始说普通话，识别结束后可编辑，不会自动发送。',false);epoch++;request?.abort();request=null;messages=[];input='';busy=false;}
    ...
    async function send(){
      if(busy)return;if(voice.active()){notify('请先结束语音输入，核对识别文字后再发送');return;}const text=input.trim();if(!text){notify('请先输入问题或现场反馈');return;}if(text.length>2000){notify('单条问题请控制在 2000 字以内');return;}
    ...
    document.addEventListener('input',e=>{if(e.target.id==='chat-input'){if(voice.active())voice.cancel('已改为键盘输入，语音已取消。',false);input=e.target.value;}});
    ...
    document.addEventListener('click',async e=>{
      ...
      try{const action=el.dataset.ai;
        ...
        if(action==='voice'){if(busy)return;if(voice.active())voice.stop();else voice.start();return;}
        if(action==='voice-cancel'){voice.cancel();return;}
        if(action==='prompt'){if(busy||voice.active())return;input=prompts[Number(el.dataset.index)];update();document.getElementById('chat-input')?.focus();return;}
        ...
        if(action==='action'){
          const m=messages.find(m=>String(m.id)===el.dataset.message),a=m?.actions?.[Number(el.dataset.index)];if(!a||a.used||busy||voice.active())throw new Error('请先结束语音或等待当前操作完成');if(m.key!==key())throw new Error('演练依据已变化，请重新提问后核对当前建议');
    ...
    function navigation(page){if(page!=='assistant'){returnTo=page;if(voice.active())voice.cancel('已切换页面，本次语音已取消，原草稿已保留。');}const floating=document.getElementById('assistant-launcher');if(floating)floating.hidden=page==='assistant';}
    root.addEventListener?.('pagehide',()=>{if(voice.active())voice.cancel('页面已离开，语音已停止。可重新点击语音输入。',false);});
    root.addEventListener?.('pageshow',()=>update());
    document.addEventListener('visibilitychange',()=>{if(document.hidden&&voice.active())voice.cancel('页面已离开前台，语音已停止。');});
