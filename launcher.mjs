...
async function probe(){try{const r=await fetch(base+'/api/v3/state',{signal:AbortSignal.timeout(600)});if(!r.ok)return 'occupied';const body=await r.json();return body.data?.schema==='jiaoying-v3'&&body.capabilities?.realtimeEvents===true&&body.capabilities?.villageReporting===true&&body.capabilities?.operations===true?'ready':'occupied';}catch(e){return e.cause?.code==='ECONNREFUSED'?'absent':'occupied';}}
